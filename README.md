...
jobs:
  claude-response:
    ...
    steps:
      - uses: yanoty3d/claude-code-action@main  # Fork with OAuth support
        with:
...
steps:
  - uses: yanoty3d/claude-code-action@main
    with:
      direct_prompt: |
        Update the API documentation in README.md to reflect
        the changes made to the API endpoints in this PR.
...
This action is built on top of [`yanoty3d/claude-code-base-action`](https://github.com/yanoty3d/claude-code-base-action).
